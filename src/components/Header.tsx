...
import { Building2, ChevronDown, Crown, LogOut, Mail, Menu, Moon, Phone, Settings, ShoppingCart, Sun, User } from 'lucide-react'
          ...
          <DropdownMenu open={profileOpen} onOpenChange={(open) => { setProfileOpen(open); if (!open) setContactOpen(false) }}>
            ...
            <DropdownMenuContent align="end" className="w-[min(340px,calc(100vw-24px))] overflow-hidden rounded-3xl border-emerald-950/10 p-0 shadow-[0_22px_65px_-18px_rgba(6,78,59,.35)] data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:slide-in-from-top-2 dark:border-white/10 dark:bg-[#0b1915]">
              ...
              <div className="p-2">
                ...
                {contactOpen && <div className="mx-2 mb-2 space-y-2 rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-xs dark:border-emerald-400/15 dark:bg-emerald-400/[.07]">
                  <div className="flex items-center gap-2 text-slate-600 dark:text-slate-300"><User className="size-3.5 shrink-0 text-emerald-600" /><span className="truncate">{user?.name ?? 'Нэр бүртгэлгүй'}</span></div>
                  {user?.email ? <a href={`mailto:${user.email}`} className="flex items-center gap-2 text-slate-600 hover:text-emerald-700 dark:text-slate-300"><Mail className="size-3.5 shrink-0 text-emerald-600" /><span className="truncate">{user.email}</span></a> : <div className="flex items-center gap-2 text-slate-400"><Mail className="size-3.5 shrink-0 text-emerald-600" />Имэйл бүртгэлгүй</div>}
                  {user?.phone ? <a href={`tel:${user.phone}`} className="flex items-center gap-2 text-slate-600 hover:text-emerald-700 dark:text-slate-300"><Phone className="size-3.5 shrink-0 text-emerald-600" /><span className="truncate">{user.phone}</span></a> : <div className="flex items-center gap-2 text-slate-400"><Phone className="size-3.5 shrink-0 text-emerald-600" />Утас бүртгэлгүй</div>}
                </div>}
