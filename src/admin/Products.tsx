...
export default function AdminProducts() {
  ...
  const products = useQuery({
    ...
    queryFn: async () =>
      (
        await apiClient.get<ProductPage>("/products/manage", {
          params: { page, pageSize: 20, q: appliedSearch || undefined },
        })
      ).data,
  });
  const categories = useQuery({
    ...
    queryFn: async () =>
      (await apiClient.get<Category[]>("/catalog/categories")).data,
  });
  const refresh = async () => {
    await Promise.all([
      client.invalidateQueries({ queryKey: ["managed-products"] }),
  ...
  const save = useMutation({
    ...
    onSuccess: async () => {
      setMessage(
        editing
          ? "Барааны мэдээлэл шинэчлэгдлээ."
          : "Шинэ бараа амжилттай нэмэгдлээ.",
      );
      setDialogOpen(false);
      setEditing(null);
      setForm(emptyForm);
  ...
  const stats = useMemo(
    () => ({
      ...
      low: rows.filter((row) => row.active && row.stock <= 5).length,
      value: rows.reduce((sum, row) => sum + row.price * row.stock, 0),
    }),
    [products.data?.total, rows],
  );
  const exportCsv = async () => {
    const response = await apiClient.get("/catalog/export.csv", {
      responseType: "blob",
    });
    const url = URL.createObjectURL(response.data as Blob),
      link = document.createElement("a");
    link.href = url;
    link.download = "tradeflow-products.csv";
    link.click();
  ...
  return (
    <>
      <PageHeader
        eyebrow="Каталог ба нөөц"
        title="Барааны удирдлага"
        description="Барааны код, үнэ, үлдэгдэл болон ангиллыг нэг дороос удирдана. Барааны код зөвхөн ажилтны хэсэгт харагдана."
        actions={
          <>
            ...
            <Button
              variant="secondary"
              onClick={() => window.location.assign("/admin/catalog")}
            >
      ...
      {message && (
        <div
          className={`mb-5 rounded-2xl border p-4 text-sm font-semibold ${message.includes("алдаа") || message.includes("бүртгэлтэй") ? "border-rose-200 bg-rose-50 text-rose-700" : "border-emerald-200 bg-emerald-50 text-emerald-700"}`}
        >
          {message}
        </div>
      )}
      ...
      <Card className="overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px] text-sm">
            <thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-400">
              <tr>
                <th className="p-4">Барааны код</th>
            ...
            <tbody className="divide-y">
              {rows.map((row) => (
                <tr key={row.id} className="hover:bg-emerald-50/40">
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      <img
                        src={row.image}
                        alt=""
                        className="size-11 rounded-xl bg-slate-100 object-cover"
                      />
                      <div>
                  ...
                  <td className="p-4">
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-bold ${row.active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}
                    >
        ...
        <Pagination
          page={page}
          total={products.data?.total ?? 0}
          pageSize={products.data?.pageSize ?? 20}
        ...
        >
          <div className="pr-8">
            <h2 className="text-2xl font-bold">
              {editing ? "Барааны мэдээлэл засах" : "Шинэ бараа бүртгэх"}
            </h2>
            <p className="mt-2 text-sm text-slate-500">
              SKU бол дотоод код тул customer дэлгүүрт харагдахгүй. Нөөцийн
              тоонд одоо бэлэн байгаа ширхэгийг оруулна.
            </p>
          </div>
          <form onSubmit={submit} className="mt-6 grid gap-4 sm:grid-cols-2">
            <Field label="Барааны код (SKU)">
              <Input
                required
                maxLength={64}
                value={form.sku}
            ...
            <Field
              label={editing ? "Одоогийн нөөцийн тоо" : "Эхний нөөцийн тоо"}
            >
              <Input
                required
                min="0"
            ...
            <Field label="Барааны зураг">
              <label className={`flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed px-3 py-2 text-center text-sm font-semibold transition ${uploadImage.isPending ? 'cursor-wait bg-emerald-50 text-emerald-700' : 'hover:border-emerald-400 hover:bg-emerald-50'}`}>
                ...
                {uploadImage.isPending ? 'Зургийн чанарыг шалгаж байна...' : 'Зураг сонгож upload хийх'}
                <input
                  type="file"
              ...
              {form.image && (
                <div className="overflow-hidden rounded-2xl border bg-slate-50">
                  ...
                  <div className="border-t px-3 py-2 text-xs font-medium text-emerald-700">Сайжруулсан зураг бэлэн</div>
                </div>
              )}
            </Field>
