...
const modules = ['dashboard','catalog','pricing','procurement','inventory','orders','fulfillment','finance','reports','users','settings']
...
export default function AdminRoles() {
  ...
  return <><PageHeader eyebrow="FR-1.2" title="Бодит RBAC permission editor" description="Role бүрийн module тус бүр дээр CRUD эрхийг PostgreSQL-д хадгална." /><Card className="overflow-auto"><table className="w-full min-w-[1000px] text-sm"><thead><tr className="border-b bg-slate-50"><th className="p-4 text-left">Role / Module</th>{modules.map((module) => <th key={module} className="p-3 text-center capitalize">{module}</th>)}</tr></thead><tbody>{roles.map((role) => <tr key={role} className="border-b align-top"><td className="p-4"><div className="flex items-center gap-2 font-bold"><ShieldCheck className="size-4 text-emerald-600" />{role}</div></td>{modules.map((module) => { const row = value(role, module); return <td key={module} className="p-2"><div className="grid grid-cols-2 gap-1">{actions.map(([field, label]) => <label key={field} title={label} className="flex cursor-pointer items-center gap-1 rounded-lg p-1 text-[10px] hover:bg-slate-50"><input type="checkbox" checked={row[field]} onChange={() => save.mutate({ ...row, [field]: !row[field] })} className="accent-emerald-600" />{label.slice(0, 1)}</label>)}</div></td> })}</tr>)}</tbody></table></Card><p className="mt-3 text-xs text-slate-500">У = унших, Ү = үүсгэх, З = засах, У = устгах. Өөрчлөлт бүр автоматаар хадгалагдана.</p></>
}
