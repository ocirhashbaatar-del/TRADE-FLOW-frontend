...
export default function Home() {
  ...
  const authenticatedLink = (destination: string) => user
    ? { to: destination }
    : { to: '/auth/login', state: { from: destination } }

  const submitSearch = (event: React.FormEvent) => {
    event.preventDefault()
    const destination = query.trim() ? `/products?q=${encodeURIComponent(query.trim())}` : '/products'
    navigate(user ? destination : '/auth/login', user ? undefined : { state: { from: destination } })
  }
  ...
  return (
    <div className="marketplace-home bg-white text-slate-950 dark:bg-[#07110e] dark:text-white">
      <section ref={heroRef} className="relative min-h-[76svh] overflow-hidden rounded-b-[36px] bg-[#06140f] text-white shadow-2xl shadow-emerald-950/15">
        <motion.img initial={{ scale: 1.08, opacity: 0, y: -30 }} animate={{ scale: 1, opacity: .86, y: 0 }} transition={{ duration: 1.4, ease: [0.22, 1, 0.36, 1] }} src={assetUrl('/images/supermarket-hero-v2.png')} alt="Ус, ундаа, хөнгөн зууш, хүнс болон шинэхэн бүтээгдэхүүнтэй орчин үеийн супермаркет" className="absolute inset-0 size-full object-cover object-[62%_center] sm:object-center" />
        ...
        <motion.div style={{ y: reduceMotion ? 0 : heroContentY, opacity: reduceMotion ? 1 : heroOpacity }} className="relative mx-auto grid min-h-[76svh] max-w-[1380px] items-center gap-12 px-4 py-24 sm:px-6 lg:grid-cols-[1fr_360px] lg:px-8">
          <motion.div initial="hidden" animate="visible" variants={{ hidden: {}, visible: { transition: { staggerChildren: .12, delayChildren: .2 } } }} className="max-w-3xl">
            ...
            <motion.h1 variants={{ hidden: { opacity: 0, y: 38 }, visible: { opacity: 1, y: 0 } }} transition={{ duration: .8, ease: [0.22, 1, 0.36, 1] }} className="mt-6 max-w-3xl text-balance text-3xl font-semibold leading-[1.05] tracking-[-.05em] sm:text-6xl sm:leading-[1.02] lg:text-7xl">Шинэхэн хүнс, ус ундааг <span className="text-emerald-300">нэг дороос.</span></motion.h1>
