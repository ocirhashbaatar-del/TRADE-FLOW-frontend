...
export default function Products() {
  ...
  return <div className="min-h-screen bg-[#edf5ef] pb-28 text-slate-950 dark:bg-[#07110e] dark:text-white">
    <section className="relative min-h-[70vh] overflow-hidden bg-[#0a2a20] text-white">
      {heroSlides.map((slide, index) => <img key={slide.image} src={assetUrl(slide.image)} alt={slide.title} className={`absolute inset-0 size-full object-cover brightness-110 transition duration-700 ${heroSlide === index ? 'scale-100 opacity-100' : 'scale-105 opacity-0'}`} />)}
      <div className="absolute inset-0 bg-gradient-to-r from-[#06261c]/90 via-[#073126]/50 to-transparent" />
      <button type="button" onClick={() => setHeroSlide((heroSlide - 1 + heroSlides.length) % heroSlides.length)} className="absolute left-4 top-1/2 z-20 grid size-12 -translate-y-1/2 place-items-center rounded-full border border-white/25 bg-white/15 text-white shadow-xl backdrop-blur-xl transition hover:scale-110 hover:bg-white/25 sm:left-7" aria-label="Өмнөх banner"><ChevronLeft className="size-6" /></button>
      <button type="button" onClick={() => setHeroSlide((heroSlide + 1) % heroSlides.length)} className="absolute right-4 top-1/2 z-20 grid size-12 -translate-y-1/2 place-items-center rounded-full border border-white/25 bg-white/15 text-white shadow-xl backdrop-blur-xl transition hover:scale-110 hover:bg-white/25 sm:right-7" aria-label="Дараагийн banner"><ChevronRight className="size-6" /></button>
      <div className="relative mx-auto flex min-h-[70vh] max-w-[1380px] items-center px-16 py-16 sm:px-20 lg:px-24">
        <div className="max-w-2xl">
          ...
          <h1 className="mt-5 text-4xl font-semibold leading-[1.02] tracking-[-.05em] drop-shadow-lg sm:text-6xl lg:text-7xl">{heroSlides[heroSlide].title}<span className="mt-2 block text-amber-300">{heroSlides[heroSlide].accent}</span></h1>
    ...
    <main className="relative z-10 mx-auto -mt-10 max-w-[1380px] rounded-t-[44px] border-x border-t border-emerald-950/[.07] bg-[#f8fbf8] px-4 pb-10 pt-12 shadow-[0_-20px_70px_rgba(4,45,32,.08)] sm:px-6 lg:px-8 dark:border-white/10 dark:bg-[#07110e]">
      ...
      <div className="mt-8">{isLoading ? <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{Array.from({ length: 8 }).map((_, index) => <Skeleton key={index} className="h-[450px] rounded-[32px]" />)}</div> : visibleProducts.length === 0 ? <div className="rounded-[30px] border border-dashed border-emerald-300 bg-white py-16 dark:bg-white/5"><EmptyState title="Бүтээгдэхүүн олдсонгүй" description="Өөр хайлтын үг, ангилал эсвэл шүүлтүүр ашиглана уу." /></div> : <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{visibleProducts.map((product, index) => <ProductCard key={product.id} product={product} index={index} favorite={favorites.includes(product.id)} onFavorite={() => toggleFavorite(product.id)} />)}</div>}</div>
    </main>
  </div>
}
