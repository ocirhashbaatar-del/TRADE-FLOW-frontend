...
export default function Products() {
  const { savedProductIds: favorites, toggleSaved: toggleFavorite } = useCart()
  ...
  return <div className="min-h-screen bg-[#edf5ef] pb-28 text-slate-950 dark:bg-[#07110e] dark:text-white">
    ...
    <main className="relative z-10 mx-3 -mt-7 rounded-t-[28px] border-x border-t border-emerald-950/[.07] bg-[#f8fbf8] px-3 pb-10 pt-8 shadow-[0_-20px_70px_rgba(4,45,32,.08)] sm:mx-4 sm:-mt-10 sm:rounded-t-[44px] sm:px-6 sm:pt-12 lg:mx-6 lg:px-6 dark:border-white/10 dark:bg-[#07110e]">
      ...
      <div className="mt-6 sm:mt-8">{isLoading ? <div className="grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-3 xl:grid-cols-5">{Array.from({ length: 10 }).map((_, index) => <Skeleton key={index} className="h-[360px] rounded-[20px] sm:h-[450px] sm:rounded-[32px]" />)}</div> : visibleProducts.length === 0 ? <div className="rounded-[30px] border border-dashed border-emerald-300 bg-white py-16 dark:bg-white/5"><EmptyState title="Бүтээгдэхүүн олдсонгүй" description="Өөр хайлтын үг, ангилал эсвэл шүүлтүүр ашиглана уу." /></div> : <div className="grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-3 xl:grid-cols-5">{visibleProducts.map((product, index) => <ProductCard key={product.id} product={product} index={index} favorite={favorites.includes(product.id)} onFavorite={() => toggleFavorite(product.id)} />)}</div>}</div>
    </main>
  </div>
}
